...
Add a child page to the Home page by clicking the + icon (Add child page) at the top of the screen.
...
Basic blog index page with three blogs listed, with their title, content
...
image is a ForeignKey to Wagtail’s built-in Image model, which stores the actual images. This appears in the page editor as a pop-up interface for choosing an existing image or uploading a new one. This way, you allow an image to exist in multiple galleries. This creates a many-to-many relationship between pages and images.

Specifying on_delete=models.CASCADE on the foreign key means that deleting the image from the system also deletes the gallery entry. In other situations, it might be appropriate to leave the gallery entry in place. For example, if an “our staff” page includes a list of people with headshots, and you delete one of those photos, but prefer to leave the person in place on the page without a photo. In this case, you must set the foreign key to blank=True, null=True, on_delete=models.SET_NULL.
...
"Second Post" page, with title, date, intro, body, and a gallery of three images
...
"Blog" page, with blog information and authors field
...
"Blog" page, with authors presented as a checklist
...
"Second Post" page, with title, date, authors, intro, body, and a gallery of three images
...
Screenshot of the "Second Post" page in the editor form, showing the Content tab
...
A page titled "Showing pages tagged bread", with two page links underneath
